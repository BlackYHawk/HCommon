...
public class StringUtil {
     ...
     */
    public static String formatInfoTime(String time) {
        String display = "";
        ...
        if (time != null) {
            try {
                if (time.length() == 14) {
                    ...
                    display = new SimpleDateFormat(Pattern5, Locale.getDefault()).format(tDate);
                }
     ...
     * @return time为null，或者时间格式不匹配，输出空字符""
     */
    public static String formatOpinionTime(String time) {
        String display = "";
        ...
        if (time != null) {
            try {
                if (time.length() == 14) {
                    Date tDate = new SimpleDateFormat(Pattern2, Locale.getDefault()).parse(time);// 格式化输入的时间
                    display = new SimpleDateFormat(Pattern8, Locale.getDefault()).format(tDate);
                }
                else if (time.length() == 12) {
                    Date tDate = new SimpleDateFormat(Pattern6, Locale.getDefault()).parse(time);// 格式化输入的时间
                    display = new SimpleDateFormat(Pattern8, Locale.getDefault()).format(tDate);
